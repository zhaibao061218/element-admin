  ...
  <div class="transferSlipYN">
    ...
    >
      ...
      <el-form-item prop="startAmount">
        <el-input v-model="form.startAmount" @change="handleMinChange" />
      </el-form-item>
      ~
      <el-form-item prop="endAmount">
        <el-input v-model="form.endAmount" @change="handleMaxChange" />
      </el-form-item>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="CurrencyName" label="币别"></el-table-column>
      <el-table-column label="调拨金额">
        <template slot-scope="scope">
          {{ scope.row.CurrencySymbol }}{{ scope.row.TransferAmount }}
        </template>
      </el-table-column>
      <el-table-column
        prop="BillingAccountTypeName"
        label="出账账户"
      ></el-table-column>
      <el-table-column
    ...
    >
      <div>调拨单单单号：{{ auditObj.OrderNo }}</div>
      <el-row :gutter="10" style="margin-top: 20px">
        <el-col :span="11" :offset="1">
          ...
          <div class="tableTitle">
            <div>出账账户</div>
            <div>{{ auditObj.BillingAccountTypeName }}</div>
          </div>
          <div class="tableTitle">
            <div>添加时间</div>
        ...
        <el-col :span="11" :offset="1">
          <div class="tableTitle">
            <div>币别</div>
            <div>{{ auditObj.CurrencyName }}</div>
          </div>
          <div class="tableTitle">
            <div>调拨金额</div>
            <div>
              {{ auditObj.CurrencySymbol }}{{ auditObj.TransferAmount }}
            </div>
          </div>
          <div class="tableTitle">
            <div>入账账户</div>
            <div>{{ auditObj.CreditAccountTypeName }}</div>
          </div>
          <div class="tableTitle">
            <div>添加人</div>
...
<script>
import {
  get_transferorder_list,
  get_transferorder_detailView,
  put_approval_transferpayorder,
} from "@/api/bills";
export default {
  data() {
    return {
      ...
      auditObj: {
        ApprovalStatus: null, //
        ApprovalStatusName: null, // "待审核"
        ApprovalUserName: null, // null
        BillingAccountType: null, // 1
        BillingAccountTypeName: null, // "余额账户"
        BuyerId: null, // 337
        BuyerUserName: null, // "sheenadhillon1@yahoo.com"
        CreateTime: null, // "2021-03-09 14:32:26.0000"
        CreateUserName: null, // "胡晓婷"
        CreditAccountType: null, // 2
        CreditAccountTypeName: null, // "付错币别"
        Currency: null, // 1
        CurrencyName: null, // "USD"
        CurrencySymbol: null, // "$"
        Id: null, // 2
        OrderNo: null, // null
        Remark: null, // null
        Summary: null, // "asd"
        TransferAmount: null, // 100
      },
  ...
  methods: {
    ...
    getTransferSlipList() {
      ...
      get_transferorder_list(this.form).then((res) => {
    ...
    refuse() {
      ...
      put_approval_transferpayorder(param).then((res) => {
        if (res.Data.Result) {
          this.$message({
            message: "审核成功",
            type: "success",
    ...
    handleEdit(index, row) {
      ...
      get_transferorder_detailView({ id: row[index].Id }).then((res) => {
    ...
    //对表单进行重新验证
    handleMinChange() {
      this.$refs.form.validateField("startAmount");
    },
    //表单重置
    resetForm(formName) {
    ...
    //对表单进行重新验证
    handleMaxChange() {
      this.$refs.form.validateField("endAmount");
    },
    //验证用户输入值
    ...
    validateMin(rule, value, callback) {
      const one = Number(value);
      const max = Number(this.form.endAmount);
      if (!max || one < max) {
        return callback();
      }
    ...
    validateMax(rule, value, callback) {
      const one = Number(value);
      const min = Number(this.form.startAmount);
      if (!min || one > min) {
        return callback();
      }
